...
async def csv_headers(
    ...
    async with _aiofiles.open(path, mode=mode, encoding=encoding, newline=newline) as afp:
        return await csv_file_headers(afp, mode, encoding, newline, delimiter)
...
async def csv_to_records(
    ...
    """
    async with _aiofiles.open(path, mode=mode, encoding=encoding, newline=newline) as afp:
        async for row in csv_file_to_records(afp, mode, encoding, newline, delimiter):
            yield row
...
async def csv_file_to_records_chunks(
    ...
    """
    records = csv_file_to_records(async_file, mode, encoding, newline, delimiter)
...
async def csv_to_records_chunks(
    ...
    async with _aiofiles.open(path, mode=mode, encoding=encoding, newline=newline) as f:
        async for chunk in csv_file_to_records_chunks(f, chunk_size, mode, encoding, newline, delimiter):
            yield chunk